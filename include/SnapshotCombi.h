 ...
 * - Uses Polymorphic Buffers to guarantee stable memory addresses and eliminate 
 *   std::variant branch overhead during the hot write loop.
 */
...
namespace io {
    ...
    // =========================================================================
    // POLYMORPHIC BUFFER ARCHITECTURE
    // =========================================================================

    /**
     * @brief Base interface for type-erased columnar buffers.
     * @details Replaces std::variant to eliminate switch statement overhead in the hot loop.
     *          Provides exhaustive overloads to guarantee compile-time resolution.
     */
    class ColumnBufferBase {
    public:
        virtual ~ColumnBufferBase() = default;
        
        /** @brief Links the internal scalar memory to the TTree branch. */
        virtual void BindToTree(TTree* tree, const std::string& name) = 0;

        // Exhaustive overloads to prevent ambiguous template resolution for any input type
        virtual void SetValue(double v) = 0;
        virtual void SetValue(float v) = 0;
        virtual void SetValue(int v) = 0;
        virtual void SetValue(unsigned int v) = 0;
        virtual void SetValue(short v) = 0;
        virtual void SetValue(unsigned short v) = 0;
        virtual void SetValue(bool v) = 0;
        virtual void SetValue(long v) = 0;
        virtual void SetValue(unsigned long v) = 0;
        virtual void SetValue(long long v) = 0;
        virtual void SetValue(unsigned long long v) = 0;
    };

    /**
     * @brief Strongly typed column buffer for TTree writing.
     * @tparam T The native ROOT data type of the column.
     */
    template <typename T>
    class ColumnBuffer : public ColumnBufferBase {
    public:
        void BindToTree(TTree* tree, const std::string& name) override;

        void SetValue(double v) override;
        void SetValue(float v) override;
        void SetValue(int v) override;
        void SetValue(unsigned int v) override;
        void SetValue(short v) override;
        void SetValue(unsigned short v) override;
        void SetValue(bool v) override;
        void SetValue(long v) override;
        void SetValue(unsigned long v) override;
        void SetValue(long long v) override;
        void SetValue(unsigned long long v) override;
    ...
    private:
        ...
        // 2D Vector holding our pre-allocated, thread-local polymorphic buffers
        std::vector<std::vector<std::unique_ptr<rad::io::ColumnBufferBase>>> _buffers; 
        ...
        template <typename T> auto get_flat_val(const T& input, int idx);
        template <typename T> auto get_flat_val(const ROOT::VecOps::RVec<T>& input, int idx);
    };
    ...
    // --- ColumnBuffer Implementations ---
    template <typename T> inline void ColumnBuffer<T>::BindToTree(TTree* tree, const std::string& name) {
        // Safe binding: The unique_ptr guarantees this memory address never moves.
        tree->Branch(name.c_str(), &_val);
    }
    
    template <typename T> inline void ColumnBuffer<T>::SetValue(double v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(float v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(int v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(unsigned int v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(short v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(unsigned short v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(bool v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(long v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(unsigned long v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(long long v) { _val = static_cast<T>(v); }
    template <typename T> inline void ColumnBuffer<T>::SetValue(unsigned long long v) { _val = static_cast<T>(v); }

    // --- SnapshotCombi Implementations ---
    ...
    inline std::shared_ptr<unsigned long> SnapshotCombi::GetResultPtr() const { return _totalCount; }
    ...
    inline void SnapshotCombi::InitTask(TTreeReader*, unsigned int slot) {
        ...
        if (!_files[slot]) {
            // Pre-allocate the vector of unique pointers for this thread
            _buffers[slot].resize(_colNames.size());
            ...
            // Evaluate types ONCE during setup, completely eliminating the hot-loop switch
            for(size_t i=0; i<_colNames.size(); ++i) {
                switch(_colTypes[i]) {
                    case ColType::Double: _buffers[slot][i] = std::make_unique<ColumnBuffer<Double_t>>(); break;
                    case ColType::Float:  _buffers[slot][i] = std::make_unique<ColumnBuffer<Float_t>>();  break;
                    case ColType::Int:    _buffers[slot][i] = std::make_unique<ColumnBuffer<Int_t>>();    break;
                    case ColType::UInt:   _buffers[slot][i] = std::make_unique<ColumnBuffer<UInt_t>>();   break;
                    case ColType::Short:  _buffers[slot][i] = std::make_unique<ColumnBuffer<Short_t>>();  break;
                    case ColType::Bool:   _buffers[slot][i] = std::make_unique<ColumnBuffer<Bool_t>>();   break;
                    case ColType::Long:   _buffers[slot][i] = std::make_unique<ColumnBuffer<Long64_t>>(); break; 
                    case ColType::Long64: _buffers[slot][i] = std::make_unique<ColumnBuffer<Long64_t>>(); break;
                    default:              _buffers[slot][i] = std::make_unique<ColumnBuffer<Double_t>>(); break;
                }
                
                // Bind the initialized buffer directly to the TTree branch
                _buffers[slot][i]->BindToTree(_trees[slot].get(), _colNames[i]);
            }
        }
    }
    ...
    inline void SnapshotCombi::set_value(unsigned int slot, int idx, const T& input) {
        ...
        // The compiler deduces `val`'s exact type and triggers the correct virtual SetValue overload, 
        // writing directly to the memory address bound to the TTree branch.
        _buffers[slot][I]->SetValue(val);
    }
