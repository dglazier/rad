...
// namespace rad { namespace config { class ConfigReaction; } } // Forward declaration if only used as pointer
// namespace rad { namespace utils { std::string createFunctionCallStringFromVec(const std::string& func_name, const std::vector<std::string>& args); } }
// namespace rad { namespace epic { /* Function ModifyMomentum, assumed to exist and take RVecs by non-const reference */ } }

// It's good practice to wrap your code in a namespace
// namespace rad { // Example namespace
...
// } // namespace rad
    ...
    public:
      ...
      ParticleModifier() = default;
    ParticleModifier(rad::config::ConfigReaction& cr):_reaction{&cr}{};

      void Apply(const string& name){
	if(_modifications.empty()==true) return;

	//loop over all added modifications
	//and combine into a Filter string
	std::string apply_all="(";
	for(auto& func:_modifications){
	  apply_all+=func;
	  apply_all+="+";
	}
	apply_all.pop_back(); //remove last +
	apply_all+=")*false + true"; //guarantee evaluate to true

	std::cout<<"ParticleModifier::Apply "<<name<<" "<<apply_all<<std::endl;
	//can now apply all modifications via one filter operation
	//this forces the modifications to be applied now!
	_reaction->Filter(apply_all,name);

      }
 
       void ChangeRecMomentumOfTo(const string& particle,const string& new_mag){
	 std::vector<std::string> vars={particle,new_mag,"rec_px","rec_py","rec_pz"};
	 auto func = rad::utils::createFunctionCallStringFromVec("rad::epic::ModifyMomentum",vars);
	_modifications.push_back(func);
      }
   
    private:
      
      mutable rad::config::ConfigReaction* _reaction=nullptr;
      std::vector<std::string> _modifications;
